...
# Data source to read the linux function app names from the file
data "local_file" "linux_function_app_serviceplan" {
  count    = fileexists("linux_function_app_serviceplan.txt") ? 1 : 0
  filename = "${path.module}/linux_function_app_serviceplan.txt"
}

# Data source to read the windows function app names from the file
data "local_file" "windows_function_app_serviceplan" {
  count    = fileexists("windows_function_app_serviceplan.txt") ? 1 : 0
