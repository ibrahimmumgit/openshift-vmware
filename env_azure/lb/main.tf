resource "azurerm_public_ip" "pubip01" {
...
resource "azurerm_public_ip" "pubip02" {
  count               = var.is_public == true && var.frontend_count == 2 ? 1 : 0
  name                = (format("${var.rg_prefix}-%s%02d", local.effective_environment == "PROD" ? "1${local.effective_applicationname}pubip" : local.effective_environment == "UAT" ? "4${local.effective_applicationname}pubip" : "5${local.effective_applicationname}pubip", local.existing_pubip_max_resource_numbers + 2))
  resource_group_name = var.create_new_resource_group ? azurerm_resource_group.rg[0].name : data.azurerm_resource_group.rg[0].name
  location            = var.create_new_resource_group ? azurerm_resource_group.rg[0].location : data.azurerm_resource_group.rg[0].location
  sku                 = "Standard"
  sku_tier            = "Regional"
...
module "azlb" {
  ...
  public_ip_address_id1 = var.is_public == true ? azurerm_public_ip.pubip01[0].id : null
  ...
  public_ip_address_id2 = var.is_public == true && var.frontend_count == 2 ? azurerm_public_ip.pubip02[0].id : null
}
