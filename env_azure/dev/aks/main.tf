...
resource "azurerm_kubernetes_cluster" "aks" {
  ...
  node_resource_group     = lower("ptsg${(local.effective_environment == "PROD") ? "-1${local.effective_applicationname}${local.next_aks_name}" : local.effective_environment == "UAT" ? "-4${local.effective_applicationname}${local.next_aks_name}" : "-5${local.effective_applicationname}${local.next_aks_name}"}-aks-nodes")
