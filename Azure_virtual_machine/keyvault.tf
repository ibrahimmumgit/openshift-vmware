...
# # Create an key in the Key Vault
...
#   key_type     = "RSA"
#   key_size     = 2048
#   #key_opts     = ["decrypt", "encrypt", "sign", "wrapKey"]
...
resource "azurerm_key_vault_secret" "kvs" {
  name         = lower("ptazsg${var.environment == "PROD" ? "-1${local.effective_applicationname}-${var.applicationtype}${local.next_vm_name}-kvs" : var.environment == "UAT" ? "-4${local.effective_applicationname}-${var.applicationtype}${local.next_vm_name}-kvs" : "-5${local.effective_applicationname}-${var.applicationtype}${local.next_vm_name}-kvs"}")
...
resource "azurerm_key_vault_access_policy" "disk_encryption_access_policy" {
  ...
  key_vault_id            = data.azurerm_key_vault.cmk_vault.id #var.create_new_resource_group || var.create_new_key_vault ? azurerm_key_vault.vault[0].id : data.azurerm_key_vault.vault[0].id
  tenant_id               = data.azurerm_client_config.current.tenant_id
  object_id               = azurerm_disk_encryption_set.disk_encryption[0].identity[0].principal_id # This gets the principal ID of the disk encryption set
  certificate_permissions = ["Backup", "Create", "Delete", "DeleteIssuers", "Get", "GetIssuers", "Import", "List", "ListIssuers", "ManageContacts", "ManageIssuers", "Purge", "Recover", "Restore", "SetIssuers", "Update"]
  key_permissions         = ["Backup", "Create", "Decrypt", "Delete", "Encrypt", "Get", "Import", "List", "Purge", "Recover", "Restore", "Sign", "UnwrapKey", "Update", "Verify", "WrapKey", "Release", "Rotate", "GetRotationPolicy", "SetRotationPolicy"]
  secret_permissions      = ["Backup", "Delete", "Get", "List", "Purge", "Recover", "Restore", "Set"]
