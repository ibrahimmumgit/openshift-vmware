...
resource "azurerm_disk_encryption_set" "disk_encryption" {
  ...
  name                = lower("ptazsg${var.environment == "PROD" ? "-1${local.effective_applicationname}-${var.applicationtype}${local.next_vm_name}-de" : var.environment == "UAT" ? "-4${local.effective_applicationname}-${var.applicationtype}${local.next_vm_name}-de" : "-5${local.effective_applicationname}-${var.applicationtype}${local.next_vm_name}-de"}")
