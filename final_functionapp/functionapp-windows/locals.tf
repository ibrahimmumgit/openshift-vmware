locals {
  ...
  nwindows_funcapp_names = local.windows_function_app_names_count_from_file < var.windows_funcapp_count ? [for i in range(var.windows_funcapp_count - local.windows_function_app_names_count_from_file) : (format("${var.rg_prefix}-%s%02d", local.effective_environment == "PROD" ? "1${local.effective_applicationname}func" : local.effective_environment == "UAT" ? "4${local.effective_applicationname}func" : "5${local.effective_applicationname}func", local.existing_funcapps_max_resource_numbers + i + 1))] : []
